var showData = function() {
    ...
    var html_to_insert = '<div class="g-labels"></div>    <svg class="g-nodes" width="1070" height="640"></svg>';
    ...
    function updateNodes() {
        node = node.data(data.display, function(d) { return d.name; });
        ...
        var leftEnter = nodeEnter.append("g")
            .attr("class", "g-left");

        leftEnter.append("clipPath")
            .attr("id", function(d) { return "g-clip-left-" + d.id; })
            .append("rect");

        leftEnter.append("circle");

        var rightEnter = nodeEnter.append("g")
            .attr("class", "g-right");

        rightEnter.append("clipPath")
            .attr("id", function(d) { return "g-clip-right-" + d.id; })
            .append("rect");
        ...
        node.select(".g-left rect")
            .style("display", function(d) { return d.k > 0 ? null : "none" })
            .attr("x", function(d) { return -d.r - clipPadding; })
            .attr("width", function(d) { return 2 * d.r * d.k + clipPadding; });

        node.select(".g-right rect")
            .style("display", function(d) { return d.k < 1 ? null : "none" })
            .attr("x", function(d) { return -d.r + 2 * d.r * d.k; })
            .attr("width", function(d) { return 2 * d.r; });

        node.select(".g-left circle")
            .attr("clip-path", function(d) { return d.k < 1 ? "url(#g-clip-left-" + d.id + ")" : null; });

        node.select(".g-right circle")
            .attr("clip-path", function(d) { return d.k > 0 ? "url(#g-clip-right-" + d.id + ")" : null; });
    ...
    function updateLabels() {
        label = label.data(data.display, function(d) { return d.name; });
        ...
        label
            ...
            .select(".g-value")
            .text(function(d) { return formatShortCount(d.left) + " - " + formatShortCount(d.right); });
...
function initilizeDropdowns() {
    ...
    var dropdowns = [document.getElementById("left"), document.getElementById("right")];
